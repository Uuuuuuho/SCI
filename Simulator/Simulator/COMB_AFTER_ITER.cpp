#include "Simulator.h"
#if NOW == COMB_AFTER_ITER //Conventional Superposition
#include <iostream>
#include <fstream>	
#include <vector>
#include <string>
...
void Sim() {
	...
	double** ConvLLR, **LLR1, **LLR2, **DF_LLR1, **DF_LLR2, **SUPER_LLR1, **SUPER_LLR2;

	int g[2] = { 0x0D,0x0B };//trellis polynomial for convolutional
	llr0 = NULL, llr1 = NULL, ConvLLR = NULL, LLR1 = NULL, LLR2 = NULL, SUPER_llr0 = NULL, SUPER_llr1 = NULL, DF_LLR1 = NULL, DF_LLR2 = NULL,
	...
	case TURBO:
		...
		DF_LLR1 = (double**)malloc(sizeof(double) * (Size + turb.m_Nmemory));
		DF_LLR2 = (double**)malloc(sizeof(double) * (Size + turb.m_Nmemory));
		for (int i = 0; i < (Size + turb.m_Nmemory); i++) {
			DF_LLR1[i] = (double*)malloc(SP_NCODE * sizeof(double));
			DF_LLR2[i] = (double*)malloc(SP_NCODE * sizeof(double));
		}
	...
	do {
		...
		for (int h = 0; h < Frame; ++h)
		{
			...
			if (err > ((double)Frame * (double)Size * EARLYSTOPRATE)) {
#if (OUTPUT == SOURCE_ONLY) || (OUTPUT == SOURCE_RELAY_BOTH)
				soFar = h + count;
...
#if (OUTPUT == SOURCE_ONLY) || (OUTPUT == SOURCE_RELAY_BOTH)
		Ch == TURBO ? BER = (double)err / (((double)Frame + (double)count) * Size) : (double)err / ((double)Frame * (double)Convsize);
		soFar = (Frame + count);	//when EARLYSTOP NOT NEEDED
									//when EARLYSTOP NEEDED
	EARLYSTOP:Ch == TURBO ? BER = (double)err / ((double)(soFar) * (double)Size) : (double)err / ((double)Frame * (double)Convsize);
#elif (OUTPUT == RELAY_ONLY)
