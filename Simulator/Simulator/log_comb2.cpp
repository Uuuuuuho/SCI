...
void Sim() {
	...
	//=============================initiallization=============================================================================
	double Eb = 0, RELAY_Eb = 0, BER, PER, SNR_SR, SNR_RD, LC, LC_SD, LC_SR, LC_RD, LC_Comb, rate1, rate2;
	int err, perr, count;
	bool DEC_flag = false;
	SNR = SNRinit;
	...
	do {
		...
		for (int h = 0; h < Frame; ++h)
		{
			...
			case(QPSK): {
				...
				SOURCE_Map.QPSK_Mapping(encoded_source, SOURCE_TX);
			...
			case TURBO:
				LC_SD = -1.0 / (2 * Awgn.sigma2);
				turb.turbo_llr_generation(Fad_Mod, rx_source, llr_wgt_sd, llr0, llr1, &Map, rx_source.size(), LC_SD);
			...
			if (Detect.Packet(code_source, decoded_source, SP_NINFOBITperSYM)) {    //CRC fail @ the gateway
				//Decoding @ Relay
				LC_SR = -1.0 / (2 * AWGN2.sigma2);
				turb.turbo_llr_generation(Fad_Mod, SR_RX, LLR_SR, RELAY_llr0, RELAY_llr1, &Map, SR_RX.size(), LC_SR);
				...
				if (Detect.Packet(code_source, decoded_source, SP_NINFOBITperSYM)) {	//decoding succeeded @ Relay
					...
					LC_RD = -1.0 / (2 * AWGN3.sigma2);	//AWGN3으로 해야함
					turb.turbo_llr_generation(Fad_Mod, RD_RX, LLR_RD, SUPER_llr0, SUPER_llr1, &SOURCE_Map, RD_RX.size(), LC_RD);
					LC_Comb = Comb.Rate(SNR, RD_Gain, llr_wgt_sd, LLR_RD, LC_SD, LC_RD, rate1, rate2);	//define rate
					Comb.LLR_COMB4(LC_Comb, rate1, rate2, llr0, llr1, SUPER_llr0, SUPER_llr1, RD_RX.size());
					//Comb.LLR_COMB2(Fad_Mod, SNR, llr_wgt_sd, llr0, llr1, SNR + RD_Gain, LLR_RD, SUPER_llr0, SUPER_llr1, RD_RX.size());
					turb.turbo_bit2sym(llr0, llr1, SUPER_LLR1, SUPER_LLR2, SP_NCODEBITperSYM, NCODEBIT, SP_NCODE);
