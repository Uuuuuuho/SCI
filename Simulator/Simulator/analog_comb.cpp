...
void Sim() {
	...
	SOURCE_Map.map_tab_gen(0.75);

	RELAY_Map.init(Mod_int);
	RELAY_Map.map_tab_gen(0.25);
	...
	int err, perr, count;
	bool DEC_flag = true, DEC_R_flag = true;
	SNR = SNRinit;
	...
	do {
		...
		for (int h = 0; h < Frame; ++h)
		{
			...
			case(QPSK): {
				...
				SOURCE_Map.QPSK_Mapping(encoded_source, SOURCE_TX, 0.7);
				RELAY_Map.QPSK_Mapping(encoded_relay, RD_TX, 0.3);	//Relay의 codeword를 small energy modulation해서 더하자.
			...
			if (Detect.Packet(code_source, decoded_source, SP_NINFOBITperSYM)) {    //CRC fail @ the gateway
				//Decoding @ Relay
				LC = -1.0 / (2 * AWGN2.sigma2);
				turb.turbo_llr_generation(Fad_Mod, SR_RX, LLR_SR, RELAY_llr0, RELAY_llr1, &Map, SR_RX.size(), LC);
				turb.turbo_bit2sym(RELAY_llr0, RELAY_llr1, RELAY_LLR1, RELAY_LLR2, SP_NCODEBITperSYM, NCODEBIT, SP_NCODE);
				decoded_source = turb.turbo_decoding(RELAY_LLR1, RELAY_LLR2, ITR_RELAY);
				DEC_R_flag = Detect.Packet(code_source, decoded_source, SP_NINFOBITperSYM);

				if (!DEC_R_flag) {	//decoding succeeded @ Relay
					//RE_TX @ Relay & Combining
					//Comb.COOP_MRC(Fad_Mod, SNR, rx_source, llr_wgt_sd, SNR + RD_Gain, RD_RX, LLR_RD);   //analog combining
					Comb.SUPER_MRC(Fad_Mod, SNR, rx_source, llr_wgt_sd, SNR + RD_Gain, RD_RX, LLR_RD);

					LC = -1.0 / (2 * AWGN3.sigma2);
					turb.turbo_llr_generation(Fad_Mod, rx_source, llr_wgt_sd, SUPER_llr0, SUPER_llr1, &SOURCE_Map, rx_source.size(), LC);
					...
					DEC_flag = Detect.Packet(code_source, decoded_source, SP_NINFOBITperSYM);	//check whether source info bit decoded correctly
					...
					LC = -1.0 / (2 * Awgn.sigma2);
					turb.turbo_llr_generation(Fad_Mod, rx_source, llr_wgt_sd, SUPER_llr0, SUPER_llr1, &Map, rx_source.size(), LC);
					turb.turbo_bit2sym(SUPER_llr0, SUPER_llr1, SUPER_LLR1, SUPER_LLR2, SP_NCODEBITperSYM, NCODEBIT, SP_NCODE);
					decoded_source = turb.turbo_decoding(SUPER_LLR1, SUPER_LLR2, ITR);
				}

                if(!DEC_flag && !DEC_R_flag)  //relay에서 decoding에 성공 && source의 info bit decoding 성공
                {   //only affecting BER
					...
					turb.turbo_bit2sym(SUPER_llr0, SUPER_llr1, LLR1, LLR2, SP_NCODEBITperSYM, NCODEBIT, SP_NCODE);
