...
double SNR = 0, SNRinit, Increment = 0, SNRMAX = 0, rate = 0, SB1_Gain = 0, SB2_Gain = 0, SR_Gain = 0, RB1_Gain = 0, RB2_Gain = 0, RD_Gain = 0;	//c는 QPSK를 위한 값
...
void Sim() {
	...
	Rayleigh Ray;
	AWGN AWGN_SB1, AWGN_SB2, AWGN_SR, AWGN_RB1, AWGN_RB2, AWGN_RE;
	Detector Detect;
	Comb Comb;
	Conv Conv;
	Turb turb;
	...
	do {
		...
		for (int h = 0; h < Frame; ++h)
		{
			...
			case(QPSK): {
				...
				SOURCE_Map.QPSK_Mapping(encoded_source, tx_SourceAtRelay);
				RELAY_Map.QPSK_Mapping(encoded_relay, tx_Relay);	//Relay의 codeword를 small energy modulation해서 더하자.
