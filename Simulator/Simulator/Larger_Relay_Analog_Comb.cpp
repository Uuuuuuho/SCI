#include "Simulator.h"
#if NOW == LARGER_RELAY_ANALOG_COMB
#include <iostream>
#include <fstream>	
#include <vector>
#include <string>
...
void Sim() {
	...
	SOURCE_Map.map_tab_gen(0.25);

	RELAY_Map.init(Mod_int);
	RELAY_Map.map_tab_gen(0.75);
	...
	vector<Complex<double>> tx_source, rx_source, RE_RX, SR_RX, RD_TX, TMP_RD_TX, RD_RX, SOURCE_TX,
	...
	do {
		...
		for (int h = 0; h < Frame; ++h)
		{
			...
			case(QPSK): {
				...
				SOURCE_Map.QPSK_Mapping(encoded_source, SOURCE_TX, 0.25);
				RELAY_Map.QPSK_Mapping(encoded_relay, RD_TX,0.75);	//Relay의 codeword를 small energy modulation해서 더하자.
			...
			if (Detect.Packet(code_source, decoded_source, SP_NINFOBITperSYM)) {    //CRC fail @ the gateway
				...
				if (!Detect.Packet(code_source, decoded_source, SP_NINFOBITperSYM)) {	//decoding succeeded @ Relay
					...
					count++;
					//relay BER 뽑아야함.
					DEC_flag = Detect.Packet(code_relay, decoded_relay, SP_NINFOBITperSYM); //relay decoding suc?DEC_flag = 0 : DEC_flag = 1;
				}
				...
				{   //only affecting BER
					...
					Comb.SUPER_MRC(Fad_Mod, SNR, rx_source, llr_wgt_sd, SNR , RD_RX, LLR_RD);

					LC = -1.0 / (2 * AWGN3.sigma2);
					turb.turbo_llr_generation(Fad_Mod, rx_source, LLR_RD, SUPER_llr0, SUPER_llr1, &Map, rx_source.size(), LC);
		...
		Ch == TURBO ? BER = (double)err / ((Frame + count) * Size) : BER = (double)err / (Frame*Convsize);
