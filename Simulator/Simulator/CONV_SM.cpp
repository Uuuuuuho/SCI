#include "Simulator.h"
#if NOW == CONV_SM //Conventional Superposition
#include <iostream>
#include <fstream>	
#include <vector>
#include <string>
	...
	do {
		...
		for (int h = 0; h < Frame; ++h)
		{
			...
			if (Detect.Packet(code_source, decoded_source, SP_NINFOBITperSYM)) {    //CRC fail @ the gateway
				...
				count++;
#endif
				//=====REMAIN STILL HERE CUZ UPPER PART IS ABOUT RELAY DECODING=====

				if (!Detect.Packet(code_relay, decoded_relay, SP_NINFOBITperSYM)) {
					//WHEN DECODING RELAY PART SUCCESS
