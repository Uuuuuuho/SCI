#include "Simulator.h"
#if NOW == PARTIAL_SM_PRIOR_SOURCE //Conventional Superposition
#include <iostream>
#include <fstream>	
#include <vector>
#include <string>
	...
	do {
		...
		for (int h = 0; h < Frame; ++h)
		{
			...
			case(QPSK): {
				...
				RELAY_Map.QAM_Mapping(encoded_relay, RD_TX, P_alpha);	//Relay의 codeword를 small energy modulation해서 더하자.

				tmp_RD_TX = SOURCE_TX;
				RELAY_Map.Partial_SM(RD_TX, SOURCE_TX);
				//Constellation(RD_TX);
			...
			case(Rayl_Quasi): {
				...
				Ray.slow_Rayleigh_Fading(SOURCE_TX, RD_RayFading, RD_RX, LLR_RD);
				break;
			}
			default: {
				rx_source = tx_source;
			...
			if (Detect.Packet(code_source, decoded_source, SP_NINFOBITperSYM)) {    //CRC fail @ the gateway
				...
				//여기서 combining
				//Combining해야하는데 지금은 2nd LLR decoding 정보가 제대로 출력되는지 확인하기 위해 not combining
				...
				if (!Detect.Packet(code_source, decoded_source, SP_NINFOBITperSYM)) {
					RELAY_Map.Partial_SM_Super_Sub(tmp_RD_TX, RD_RX);

					LC = -1.0 / (2 * AWGN3.sigma2);
					turb.turbo_llr_generation_Partial_SM(Fad_Mod, RD_RX, LLR_RD, llr0, llr1, &RELAY_Map, RD_RX.size(), LC);
					turb.turbo_bit2sym(llr0, llr1, LLR1, LLR2, SP_NCODEBITperSYM, NCODEBIT, SP_NCODE);
