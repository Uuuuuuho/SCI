#include "Simulator.h"
#if NOW == LLR_HARQ3    //PAM superposition
#include <iostream>
#include <fstream>	
#include <vector>
#include <string>
...
void Sim() {
	...
	SOURCE_Map.init(QUAD_4PAM);
	SOURCE_Map.map_tab_gen(P_beta);

	RELAY_Map.init(INP_4PAM);
	...
	do {
		...
		for (int h = 0; h < Frame; ++h)
		{
			...
			case(QPSK): {
				...
				SOURCE_Map.PAM_Quad_Mapping(encoded_source, SOURCE_TX, P_beta);
				RELAY_Map.PAM_InPhase_Mapping(encoded_relay, RD_TX, P_alpha);	//Relay의 codeword를 small energy modulation해서 더하자.
			...
			if (Detect.Packet(code_source, decoded_source, SP_NINFOBITperSYM)) {    //CRC fail @ the gateway
				...
				turb.turbo_llr_generation_Quad(Fad_Mod, RD_RX, LLR_RD, llr0, llr1, &SOURCE_Map, RD_RX.size(), LC);
				turb.turbo_bit2sym(llr0, llr1, LLR1, LLR2, SP_NCODEBITperSYM, NCODEBIT, SP_NCODE);
				...
				//relay information decoding
				LC = -1.0 / (2 * AWGN3.sigma2);
				turb.turbo_llr_generation_InPhase(Fad_Mod, RD_RX, LLR_RD, llr0, llr1, &RELAY_Map, RD_RX.size(), LC);
				turb.turbo_bit2sym(llr0, llr1, LLR1, LLR2, SP_NCODEBITperSYM, NCODEBIT, SP_NCODE);
				...
				count++;
#endif
				//if (!Detect.Packet(code_relay, decoded_relay, Size)) {
				//	//RELAY_Map.Super(SOURCE_TX, RD_TX);
