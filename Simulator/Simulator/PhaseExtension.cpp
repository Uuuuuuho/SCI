#include "Simulator.h"
#if NOW == PHASE_EXTENSION    //Phase superposition
#include <iostream>
#include <fstream>	
#include <vector>
#include <string>
...
void Sim() {
	...
	RELAY_Map.rotator(RELAY_Map.m_2d_map_tab, 45);  //relay mapping table rotation
	...
	do {
		...
		for (int h = 0; h < Frame; ++h)
		{
			...
			case(QPSK): {
				...
				SOURCE_Map.PSK16_Mapping(encoded_source, encoded_relay, RD_TX);

                
                //for test
//                RELAY_Map.QPSK_Mapping(encoded_relay, RD_TX);
  //              RELAY_Map.rot(RD_TX,45);
			...
			if (Detect.Packet(code_source, decoded_source, SP_NINFOBITperSYM)) {    //CRC fail @ the gateway
				...
				//source re-decoding
				RELAY_Map.PSK16_Phase_Extension(encoded_relay, RD_RX);
