#include "Simulator.h"
#if NOW == CONV_LARGE_ALPHA //Conventional Superposition
#include <iostream>
#include <fstream>	
#include <vector>
#include <string>
	...
	do {
		...
		for (int h = 0; h < Frame; ++h)
		{
			...
			if (Detect.Packet(code_source, decoded_source, SP_NINFOBITperSYM)) {    //CRC fail @ the gateway
																					//Decoding @ Relay
				//relay에서 source의 정보를 decoding
				LC = -1.0 / (2 * AWGN2.sigma2);
				turb.turbo_llr_generation(Fad_Mod, SR_RX, LLR_SR, llr0, llr1, &Map, SR_RX.size(), LC);
				turb.turbo_bit2sym(llr0, llr1, LLR1, LLR2, SP_NCODEBITperSYM, NCODEBIT, SP_NCODE);
				decoded_source = turb.turbo_decoding(LLR1, LLR2, ITR);

				if (Detect.Packet(code_source, decoded_source, SP_NINFOBITperSYM)) {	//relay에서 source의 정보를 decoding하는데 성공하면
				...
				else {	//RELAY에서 SOURCE의 정보를 decoding하는데 실패하면, SOURCE에서 재전송
					LC = -1.0 / (2 * AWGN2.sigma2);
