#include "Simulator.h"
#if NOW == POWER_ITER_SWITCH_RETRY
#include <iostream>
#include <fstream>	
#include <vector>
#include <string>
...
void Sim() {
	...
	double* SOURCE1_TX_llr0, *SOURCE1_TX_llr1, *SOURCE2_TX_llr0, *SOURCE2_TX_llr1,
		...
		*llr0, *llr1, *SUPER_llr0, *SUPER_llr1, *MERGED_llr0, *MERGED_llr1;
	double** ConvLLR, **LLR1, **LLR2;

	double * source1_llr0, * source1_llr1, * source2_llr0, * source2_llr1,
		* relay1_llr0, * relay1_llr1, * relay2_llr0, * relay2_llr1;
	...
	SOURCE1_TX_llr0 = NULL, SOURCE1_TX_llr1 = NULL, SOURCE2_TX_llr0 = NULL, SOURCE2_TX_llr1 = NULL,
		...
		source1_llr0 = NULL, source1_llr1 = NULL, MERGED_llr0 = NULL, MERGED_llr1 = NULL, source2_llr0 = NULL, source2_llr1 = NULL;
	...
	case TURBO:
		...
		MERGED_llr0 = (double*)malloc(sizeof(double) * SP_NCODEBITperSYM * 2);
		MERGED_llr1 = (double*)malloc(sizeof(double) * SP_NCODEBITperSYM * 2);
	...
	vector<double> SOURCE1_TX_LLR0(SP_NCODEBITperSYM), SOURCE1_TX_LLR1(SP_NCODEBITperSYM), SOURCE2_TX_LLR0(SP_NCODEBITperSYM), SOURCE2_TX_LLR1(SP_NCODEBITperSYM),
		...
		for (int h = 0; h < Frame; ++h)
		{
			...
			case TURBO:
				...
				if (!s1_fail_flag) {
					...
					//turb.vec2pointer(SOURCE1_TX_LLR0, SOURCE1_TX_LLR1, SOURCE1_TX_llr0, SOURCE1_TX_llr1, SP_NCODEBITperSYM);
					//turb.turbo_bit2sym(SOURCE2_TX_llr0, SOURCE2_TX_llr1, LLR1, LLR2, SP_NCODEBITperSYM, NCODEBIT, SP_NCODE);
					//decoded_source2 = turb.turbo_decoding(LLR1, LLR2, ITR);
			...
			if (s1_fail_flag || s2_fail_flag) {		//CRC fail @ the gateway
				...
				if (!s2_fail_flag) {
					...
					//turb.vec2pointer(RELAY1_TX_LLR0, RELAY1_TX_LLR1, RELAY1_TX_llr0, RELAY1_TX_llr1, SP_NCODEBITperSYM);

					//turb.turbo_bit2sym(RELAY1_TX_llr0, RELAY1_TX_llr1, LLR1, LLR2, SP_NCODEBITperSYM, NCODEBIT, SP_NCODE);
					//decoded_source1 = turb.turbo_decoding(LLR1, LLR2, ITR);

					//s1_fail_flag = Detect.Packet(code_source1, decoded_source1, SP_NINFOBITperSYM);

					//if (!s1_fail_flag) {
					//	RELAY1_Map.Super_Sub(RELAY1_TX, RELAY_RX);	//SUB S2 INFO
					//	turb.turbo_llr_generation(Fad_Mod, RELAY_RX, LLR_RD, SUPER_llr0, SUPER_llr1, &RELAY2_Map, RELAY_RX.size(), LC);

					//	Comb.LLR_MRC(Fad_Mod, SNR + RD_Gain, SUPER_llr0, SUPER_llr1, LLR_RD,
					//		SNR, RELAY2_TX_llr0, RELAY2_TX_llr1, LLR_RD, SP_NCODEBITperSYM);			//case only s1 decoded successfully
			...
			case TURBO:
				...
				Detect.Detection(code_source2, decoded_source2, err, Size);
#endif
				break;
			case UNCODED:
