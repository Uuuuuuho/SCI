...
void Sim() {
	...
	SOURCE_Map.map_tab_gen(0.1);

	RELAY_Map.init(Mod_int);
	RELAY_Map.map_tab_gen(0.9);
	...
	do {
		...
		for (int h = 0; h < Frame; ++h)
		{
			...
			case(QPSK): {
				...
				SOURCE_Map.QPSK_Mapping(encoded_source, SOURCE_TX, 0.1);
				RELAY_Map.QPSK_Mapping(encoded_relay, RD_TX, 0.9);	//Relay의 codeword를 small energy modulation해서 더하자.
