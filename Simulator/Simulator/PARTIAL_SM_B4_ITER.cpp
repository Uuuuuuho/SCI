#include "Simulator.h"
#if NOW == PARTIAL_SM_B4_ITER //Conventional Superposition
#include <iostream>
#include <fstream>	
#include <vector>
#include <string>
		...
		for (int h = 0; h < Frame; ++h)
		{
			...
			if (Detect.Packet(code_source, decoded_source, SP_NINFOBITperSYM)) {    //CRC fail @ the gateway
				...
				if (!Detect.Packet(code_relay, decoded_relay, SP_NINFOBITperSYM)) {
					...
					LC = -1.0 / (2 * AWGN3.sigma2);
					turb.turbo_llr_generation_Partial_SM(Fad_Mod, RD_RX, LLR_RD, SUPER_llr0, SUPER_llr1, &SOURCE_Map, RD_RX.size(), LC);
