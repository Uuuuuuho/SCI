...
void Sim() {
	...
	SOURCE_Map.init(QAM16);	//딱히 필요없을듯..
	SOURCE_Map.map_tab_gen(1.0);

	RELAY_Map.init(QAM16);
	...
	int err, perr, count;
	bool DEC_flag = false, DEC_R_flag = false;
	SNR = SNRinit;
	...
	vector<bool> source_int, relay_int, code_source, code_relay, encoded_source, demapped_source, decoded_source,
		tmp_relay, encoded_relay, decoded_relay;//Packet buffer
	...
	do {
		...
		for (int h = 0; h < Frame; ++h)
		{
			...
			case TURBO:
				...
				tmp_relay = turb.encode(code_relay, relay_int);

				encoded_relay = turb.Attach(encoded_source, tmp_relay);
				break;
			...
			case(QPSK): {
				...
				RELAY_Map.QAM_Mapping(encoded_relay, RD_TX);	//Relay에서 16QAM으로 전송
			...
			if (Detect.Packet(code_source, decoded_source, SP_NINFOBITperSYM)) {    //CRC fail @ the gateway
				...
				if (!DEC_R_flag) {	//decoding succeeded @ Relay
					...
					turb.turbo_llr_generation(Fad_Mod, RD_RX, llr_wgt_sd, llr0, llr1, &RELAY_Map, RD_RX.size(), LC);
					turb.turbo_bit2sym(llr0, llr1, LLR1, LLR2, SP_NCODEBITperSYM, NCODEBIT, SP_NCODE);
					decoded_source = turb.turbo_decoding(LLR1, LLR2, ITR);
				}
	 ...
	 //           if(!DEC_flag && !DEC_R_flag)  //combining relay's packet & decoding succeeded
	 //           {   //only affecting BER
